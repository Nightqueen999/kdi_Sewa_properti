    ...
    <motion.div initial="hidden" animate="show" transition={{ staggerChildren: 0.08 }} className="max-w-7xl mx-auto px-4 py-8 space-y-6">
      <motion.div variants={item} className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <div className="flex gap-2">
          <motion.button whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }} onClick={() => setShowAdminForm(!showAdminForm)} className="btn-secondary text-sm flex items-center gap-2">🛡️ Tambah Admin</motion.button>
        ...
        {showAdminForm && (
          <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} exit={{ opacity: 0, height: 0 }} className="overflow-hidden">
            <form onSubmit={handleSetAdmin} className="glass-card p-6">
              <h3 className="text-lg font-semibold text-white mb-4">🛡️ Tambah Admin Baru</h3>
              <div className="mb-4">
              ...
              {error && <div className="mb-4 px-4 py-3 rounded-xl bg-red-500/10 border border-red-500/20"><p className="text-sm text-red-400">{error}</p></div>}
              <div className="flex gap-3">
                <button type="submit" disabled={txPending} className="btn-success text-sm disabled:opacity-50">{txPending ? 'Memproses...' : '✅ Tambah Admin'}</button>
                <button type="button" onClick={() => setShowAdminForm(false)} className="btn-secondary text-sm">Batal</button>
              </div>
            </form>
          </motion.div>
        )}
