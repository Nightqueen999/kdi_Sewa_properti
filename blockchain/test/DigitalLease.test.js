...
describe("DigitalLease", function () {
  ...
  describe("Fitur Admin", function () {
    it("Harus set deployer sebagai admin", async function () {
      expect(await digitalLease.isAdmin(owner.address)).to.be.true;
    });

    it("Admin bisa menambahkan admin baru", async function () {
      await expect(
        digitalLease.setAdmin(tenant1.address, true)
      ).to.emit(digitalLease, "AdminStatusUpdated")
        .withArgs(tenant1.address, true);
      
      expect(await digitalLease.isAdmin(tenant1.address)).to.be.true;
    });

    it("Non-admin tidak bisa menambahkan admin", async function () {
      await expect(
        digitalLease.connect(tenant1).setAdmin(tenant2.address, true)
